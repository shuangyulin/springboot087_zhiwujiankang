    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="植物编号" prop="zhiwubianhao">
          <el-input v-model="ruleForm.zhiwubianhao" 
              placeholder="植物编号" clearable  :readonly="ro.zhiwubianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="植物编号" prop="zhiwubianhao">
              <el-input v-model="ruleForm.zhiwubianhao" 
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="材料编号" prop="cailiaobianhao">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="材料编号"
                  v-model="ruleForm.cailiaobianhao" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.cailiaobianhao" label="材料编号" prop="cailiaobianhao">
                    <span>{{ruleForm.cailiaobianhao}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `zhiwujiuzhiyongliaodengji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhiwujiuzhiyongliaodengji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhiwujiuzhiyongliaodengjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
